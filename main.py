from my_portfolio_web_app.model import financial_instrument, transaction, measurement
from my_portfolio_web_app.model.valuation_system import ValuationSystem, ValuationSourceFromIOLAPI
import datetime

pesos = financial_instrument.Currency(code='$', description='Pesos')
dollars = financial_instrument.Currency(code='USD', description='Dólares EEUU')
...
mirg = financial_instrument.Stock(code="MIRG", description="MIRGOR")
...
def transactions():
    ...
    pesos.save()
    mirg.save()
    t1 = transaction.Purchase(date=datetime.date(2020, 1, 17), security_quantity=20.7, financial_instrument=mirg,
    ...
    t5 = transaction.Purchase(datetime.date(2020, 3, 9), 6412, tc20, ars(2.275), "IOL", ars(74.4))
    t6 = transaction.Sale(datetime.date(2020, 3, 9), 19164, tb21, ars(0.792), "IOL", ars(77.41))
    t7 = transaction.Purchase(datetime.date(2020, 3, 10), 3411, tc20, ars(2.277), "IOL", ars(39.61))
...
def results(financial_instrument, system):
    print(financial_instrument)
    print("Precio Promedio: " + str(system.average_price_for_on(financial_instrument, today)))
    sales_result = system.sales_result_for_on(financial_instrument, today)
    print("Resultado de Ventas: " + str(sales_result))
    commissions = system.commissions_result_for_on(financial_instrument, today)
    print("Resultado por Comisiones: " + str(commissions))
    payments_result = system.payments_result_for_on(financial_instrument, today)
    print("Resultado por Renta y Dividendos: " + str(payments_result))
    price_result = system.price_difference_result_for_on_using(financial_instrument, pesos, today, valuation_system)
    print("Resultado por Diferencia de Precios: " + str(price_result))
    print("--------------------")
    print(
        "Total " + financial_instrument.code + ": " + str(sales_result + commissions + payments_result + price_result))
    print()
...
if __name__ == '__main__':
    ...
    # stock_system = StockSystem(portfolio.registered_transactions())
    # results(abev, stock_system)
    # results(arco, stock_system)
    # results(bma, stock_system)
    # results(ko, stock_system)
    # results(meli, stock_system)
    # results(ogzd, stock_system)
    # results(tgno4, stock_system)
    # results(to21, stock_system)
    # results(vist, stock_system)
    # print("Otros")
    # results(mirg, stock_system)
