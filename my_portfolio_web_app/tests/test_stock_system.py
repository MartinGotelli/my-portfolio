...
from my_portfolio_web_app.model.financial_instrument import Bond, Stock, ars
from my_portfolio_web_app.model.measurement import Measurement
from my_portfolio_web_app.model.stock_system import OpenPosition, ClosingPosition, OpenPositionCreator, StockSystem
from my_portfolio_web_app.model.transaction import Purchase, Inflow, Sale, CouponClipping, StockDividend
from my_portfolio_web_app.model.valuation_system import ValuationSystem, ValuationSourceFromDictionary

one_euro = Measurement(1, ars)
...
purchase = Purchase(date=today, security_quantity=2000, financial_instrument=ay24, price=Measurement(0.81, ars),
                    broker="IOL", commissions=10 * one_euro)
purchase_1200 = Purchase(date=today, security_quantity=1200, financial_instrument=ay24, price=Measurement(0.81, ars),
                         broker="IOL", commissions=8 * one_euro)
purchase_10000 = Purchase(date=tomorrow, security_quantity=10000, financial_instrument=ay24, price=0.78 * one_euro,
                          broker="IOL", commissions=100 * one_euro)
today_sale = Sale(date=today, security_quantity=500, financial_instrument=ay24, price=Measurement(0.825, ars),
                  broker="IOL", commissions=3.5 * one_euro)
sale = Sale(date=tomorrow, security_quantity=500, financial_instrument=ay24, price=Measurement(0.83, ars), broker="IOL",
            commissions=4 * one_euro)
sale_2000 = Sale(date=tomorrow, security_quantity=2000, financial_instrument=ay24, price=Measurement(0.84, ars),
                 broker="IOL", commissions=7.5 * one_euro)
deposit_1000 = Inflow(date=today, security_quantity=1000, financial_instrument=ars, broker="IOL")
coupon_clipping = CouponClipping(date=tomorrow, security_quantity=550, financial_instrument=ars,
                                 referenced_financial_instrument=ay24, broker="IOL", commissions=20 * one_euro)
...
class TestStockSystem(unittest.TestCase):
    ...
    def test_sales_result_for_on(self):
        ...
        self.assertEqual(stock_system.sales_result_for_on(ay24, sale.date),
                         (0.83 - 0.81) * float(sale.security_quantity) * one_euro)
    ...
    def test_sales_result_for_multiple_closing_positions(self):
        ...
        self.assertEqual(stock_system.sales_result_for_on(ay24, today),
                         (0.825 - 0.81) * float(today_sale.security_quantity) * one_euro)
        self.assertEqual(stock_system.sales_result_for_on(ay24, sale.date),
    ...
    def test_price_difference_result_for_on_using(self):
        stock_system = StockSystem([purchase])
        self.assertEqual(stock_system.open_positions[ay24], [OpenPosition(purchase)])
        self.assertEqual(stock_system.price_difference_result_for_on_using(ay24, ars, yesterday, valuation_system), 0)
